...
if(sum(vec.e)/length(vec.e)==mean(vec.e)){print("The sum and length formula and mean formula are equal.")
...
Answer: Since this vector has 6 numbers, if we want the matrix to have two rows, it will have *three* columns. The rows and columns multiplied need to equal six; therefore, the math can be performed as follows:
6 = 2 * X;
6/2 = 2*X /2;
3=X
